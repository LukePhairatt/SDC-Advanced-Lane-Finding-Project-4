...
def ApplySobelThreshold(image_rgb, gradx, grady,mag, direct):
    ...
    bin_output[((dir_binary == 1) & (grady == 1)) | ((mag_binary == 1) & (gradx == 1))] = 1  #challenge
...
def ColorChannelThreshold(img, hls_thresh, rgb_thresh):
    ...
    bin_output[ ((bin_h == 1) & (bin_s == 1)) | ((bin_l == 1) | (bin_r ==1)) ] = 1   #challenge
    return bin_output
...
def SanityCheck(LeftLineData, RightLineData, left_fit, right_fit, left_curve, right_curve, img_size, xm_per_meter, ym_per_meter):
    ...
    if(gap_mean > 2.0 and gap_mean < 3.0):
        gap_sts = True
    
    if(parallel_mean < 0.5):
        parallel_sts = True
...
class AdvanceLaneDetection():
    def __init__(self,xm_per_pixel,ym_per_pixel,line_iteration,mtx,dist,M,Minv,Sobel,hls_thresh,rgb_thresh,vertices, mode):
        ...
        self.badDetectionThresh = 3                 # request restart after badDetection going beyond this number
        self.mode = mode                            # 'live', 'average': overlay current or average best fit result
        self.oldResultDump = [0, 0, 'N/A']          # average curvature, vehicle position, direction
    ...
    def ProcessorPipline(self, image):
        ...
        bin_output[ ((bin_color == 1) | (bin_color == 1))] = 1     # challenge
        ...
        if not found_line[0] and not found_line[1]:
            # on or two line missing
            print("Right line detection missing..Old best fit result will be displayed")
            # Assign old result
            avg_curverad_n = self.oldResultDump[0]
            avg_vehicle_off = self.oldResultDump[1]
            avg_direction = self.oldResultDump[2]
        else:
            try:
                ####
                ...
                self.oldResultDump[0] = avg_curverad_n
                self.oldResultDump[1] = avg_vehicle_off
                self.oldResultDump[2] = avg_direction
        
            except:
                # missing some detection
                # display the old result or none
...
if __name__ == '__main__':
    ...
    hls_thresh = [(15,80),(200,255),(20,255)]           # challenge
    rgb_thresh = [(220,255),(None,None),(None,None)]
    
    # Mask ROI
    vertices = np.array([[    (140, 700),
                              (612, 460), 
                              (750, 460),
                              (1100,700)
                              ]],dtype=np.int32)
    ...
    # Video processing
    drive_output = 'challenge_video_out.mp4'
